...
func TestLoad_Defaults(t *testing.T) {
	...
	assert.Equal(t, 8009, cfg.HTTPPort)
	assert.Equal(t, 9009, cfg.GRPCPort)
	assert.Equal(t, "development", cfg.Environment)
	assert.Equal(t, "localhost", cfg.PostgresHost)
	assert.Equal(t, "notification_db", cfg.PostgresDB)
}
...
func TestLoad_InvalidHTTPPort(t *testing.T) {
	t.Setenv("NOTIFICATION_HTTP_PORT", "0")
...
func TestLoad_InvalidGRPCPort(t *testing.T) {
	t.Setenv("NOTIFICATION_GRPC_PORT", "99999")
...
func TestLoad_CustomPorts(t *testing.T) {
	setEnvs(t, map[string]string{
		"NOTIFICATION_HTTP_PORT": "9090",
		"NOTIFICATION_GRPC_PORT": "9091",
	})
