...
import (
	...
	"github.com/utafrali/EcommerceGo/services/product/migrations"
	...
	handler "github.com/utafrali/EcommerceGo/services/product/internal/handler/http"
...
// App wires together all dependencies and runs the product service.
type App struct {
	cfg            *config.Config
	logger         *slog.Logger
	pool           *pgxpool.Pool
...
func NewApp(cfg *config.Config, logger *slog.Logger) (*App, error) {
	...
	database.RegisterPoolMetrics(pool, "product")
	...
	// Category, Brand & Banner repositories (simple list endpoints, no service layer needed).
	...
	// HTTP router.
	router := handler.NewRouter(productService, reviewService, categoryRepo, brandRepo, bannerRepo, healthHandler, logger, cfg.PprofAllowedCIDRs)
